...
class DatasetConfig(FairseqDataclass):
    ...
    xlmr_task: Optional[XLMR_TASK_CHOICES]  = field(
        default='encfix_embfix_decrand', metadata={"help": 'perform task related to xlmr'}
    )
    xlmr_modeldir: Optional[str] = field(
        default='xlmr_base', metadata={"help": 'perform task related to xlmr'}
    )
    ft_last_f_dir: Optional[str] = field(
        default=None, metadata={"help": 'path to load trained model of encfix-embfix-decrand model.'}
    )
    resdrop_layer: Optional[int] = field(
        default=10, metadata={"help": 'which encoder layer to remove the residual connection after self-attention. (positional disengled encoder)'}
    )
    mplm_type: Optional[str] = field(
        default='xlmr', metadata={"help": 'type of multilingual pretrained model'}
    )
    # no_lang_proj: bool = field(default=False,metadata={
    #                         'help': 'if True, do not add lang linear layer to encoder output'})
    enable_load_from_given_ckpt: bool = field(default=False,metadata={
                            'help': 'if True, do not add lang linear layer to encoder output'})
    enable_lang_proj: bool = field(default=False,metadata={
                            'help': 'if True, do not add lang linear layer to encoder output'})
    disable_prefetch: bool = field(default=False,metadata={
                            'help': 'if True, do not add lang linear layer to encoder output'})
    real_src_lang: Optional[str] = field(default=None,metadata={
                            'help': 'if True, do not add lang linear layer to encoder output'})
    real_tgt_lang: Optional[str] = field(default=None,metadata={
                            'help': 'if True, do not add lang linear layer to encoder output'})
    real_aux_src_langtok: Optional[int] = field(default=None,metadata={
                            'help': 'if True, do not add lang linear layer to encoder output'})
...
class GenerationConfig(FairseqDataclass):
    ...
    trg_pt: Optional[str] = field(
        default=None, metadata={"help": 'type of multilingual pretrained model'}
    )
    align_alignment_layer: Optional[int] = field(
        default=5, metadata={"help": 'type of multilingual pretrained model'}
    )
