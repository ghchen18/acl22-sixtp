...
class SequenceGenerator(nn.Module):
    def __init__(
        ...
        self.args = cfg   
        self.cuda_id = self.args.distributed_training.distributed_rank if hasattr(self.args, 'distributed_training') else 0
        ...
        if self.args is not None and 'vanilla' not in self.args.dataset.xlmr_task and 'mbart' not in self.args.dataset.xlmr_task:
            self.vocab_size = 250002 ## the vocabulary size for XLM-R 
        else:
            self.vocab_size =  len(tgt_dict) 
       
        self.tgt_embed_mask = None 
        ...
        self.model = self.model.cuda(f'cuda:{self.cuda_id}')
    ...
    def _generate(
        ...
        src_lang_id = int(sample['net_input']['src_lang_id'][0]) if 'src_lang_id' in sample['net_input'].keys() else None,
        tgt_lang_id = int(sample['net_input']['tgt_lang_id'][0]) if 'tgt_lang_id' in sample['net_input'].keys() else None,
