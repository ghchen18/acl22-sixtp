...
class FairseqTask(object):
    ...
    def build_generator(
        self, models, args, seq_gen_cls=None, extra_gen_cls_kwargs=None, cfg=None,
    ):
        if getattr(args, "score_reference", False):
            from fairseq.sequence_scorer_for_mmap import SequenceScorer
    ...
    def valid_step(self, sample, model, criterion, model_ref=None):
        model.eval()
        with torch.no_grad():
            loss, sample_size, logging_output = criterion(model, sample)
        return loss, sample_size, logging_output
