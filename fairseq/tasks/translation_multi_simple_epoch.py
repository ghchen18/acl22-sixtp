...
class TranslationMultiSimpleEpochTask(LegacyFairseqTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--eval-bleu', action='store_true',
                            help='keep language tokens in inference output (e.g. for analysis or debugging)')
        parser.add_argument('--eval-bleu-args', type=str, help='generation args for BLUE scoring, e.g., \'{"beam": 4, "lenpen": 0.6}\', as JSON string')
        parser.add_argument('--eval-bleu-detok', type=str, default="space", help='detokenize before computing BLEU')
        parser.add_argument('--eval-bleu-detok-args', type=str, default="{}", help='generation args for BLUE scoring, e.g., \'{"beam": 4, "lenpen": 0.6}\', as JSON string')
        parser.add_argument('--eval-tokenized-bleu', action='store_true',
                            help='keep language tokens in inference output (e.g. for analysis or debugging)')
        parser.add_argument('--eval-bleu-remove-bpe', type=str, default=None, help='generation args for BLUE scoring, e.g., \'{"beam": 4, "lenpen": 0.6}\', as JSON string')
    ...
    def __init__(self, args, langs, dicts, training):
        ...
        self.tgt_dict = self.dicts[self.target_langs[0]]

        self.sampling_method = SamplingMethod.build_sampler(args, self)
        self.data_manager = MultilingualDatasetManager.setup_data_manager(
            args, self.lang_pairs, langs, dicts, self.sampling_method, cfg=self.cfg_all,
        )
    ...
    def build_dataset_for_inference(self, src_tokens, src_lengths, constraints=None):
        ...
        src_langtok_spec, tgt_langtok_spec = self.args.langtoks["main"] if 'main' in self.args.langtoks else None, None
    ...
    def build_generator(
        ...
        return super().build_generator(
            models, args, seq_gen_cls=None, cfg=self.cfg_all, extra_gen_cls_kwargs=extra_gen_cls_kwargs
        )
    ...
    def build_model(self, args):
        ...
        if self.args.eval_bleu:
            ...
            self.sequence_generator = self.build_generator([model], Namespace(**gen_args), cfg=self.cfg)
        return model
    ...
    def valid_step(self, sample, model, criterion):
        ...
        if self.args.eval_bleu:
            src_lang = self.langs[int(sample['net_input']['src_lang_id'][0])-1]
            tgt_lang = self.langs[int(sample['tgt_lang_id'][0])-1]
            pair = f"{src_lang}-{tgt_lang}"

            if pair in self.eval_lang_pairs:
                bleu = self._inference_with_bleu(self.sequence_generator, sample, model)
                
                logging_output[f"_bleu_sys_len_{pair}"] = bleu.sys_len
                logging_output[f"_bleu_ref_len_{pair}"] = bleu.ref_len

                assert len(bleu.counts) == EVAL_BLEU_ORDER
                for i in range(EVAL_BLEU_ORDER):
                    logging_output[f"_bleu_counts_{pair}" + str(i)] = bleu.counts[i]
                    logging_output[f"_bleu_totals_{pair}" + str(i)] = bleu.totals[i]
    ...
    def get_pair_bleu(self, logging_outputs, pair):
        def sum_logs(key):
            import torch
            result = sum(log.get(key, 0) for log in logging_outputs if f"_bleu_sys_len_{pair}" in log)
        ...
        counts, totals = [], []
        for i in range(EVAL_BLEU_ORDER):
            counts.append(sum_logs(f"_bleu_counts_{pair}" + str(i)))
            totals.append(sum_logs(f"_bleu_totals_{pair}" + str(i)))

        if max(totals) > 0:
            # log counts as numpy arrays -- log_scalar will sum them correctly
            metrics.log_scalar(f"_bleu_counts_{pair}", np.array(counts))
            metrics.log_scalar(f"_bleu_totals_{pair}", np.array(totals))
            metrics.log_scalar(f"_bleu_sys_len_{pair}", sum_logs(f"_bleu_sys_len_{pair}"))
            metrics.log_scalar(f"_bleu_ref_len_{pair}", sum_logs(f"_bleu_ref_len_{pair}"))
            ...
            def compute_bleu(meters):
                ...
                bleu = sacrebleu.compute_bleu(
                    correct=meters[f"_bleu_counts_{pair}"].sum,
                    total=meters[f"_bleu_totals_{pair}"].sum,
                    sys_len=meters[f"_bleu_sys_len_{pair}"].sum,
                    ref_len=meters[f"_bleu_ref_len_{pair}"].sum,
                    **smooth
                )
                return round(bleu.score, 2)

            metrics.log_derived(f"bleu_{pair}", compute_bleu)

    def reduce_metrics(self, logging_outputs, criterion):
        super().reduce_metrics(logging_outputs, criterion)
        if self.args.eval_bleu:
            for pair in self.eval_lang_pairs:
                self.get_pair_bleu(logging_outputs, pair)
    ...
    def create_batch_sampler_func(
        ...
        def construct_batch_sampler(dataset, epoch):
            ...
            # )
            logger.info(f"[{split}] mem usage: {data_utils.get_mem_usage()}")
