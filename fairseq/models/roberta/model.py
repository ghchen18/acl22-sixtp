...
from fairseq.modules.transformer_sentence_encoder import init_bert_params, TransformerSentenceEncoder
    ...
    def forward(
        ...
        **unused
        ...
        x, extra = self.extract_features(
            src_tokens, return_all_hiddens=return_all_hiddens, extra_params = extra_params,
        )
        if not features_only:
            x = self.output_layer(x, masked_tokens=masked_tokens)
        return x, extra

    def extract_features(self, src_tokens, return_all_hiddens=False, extra_params=None, **kwargs):
        
        inner_states, encoder_out = self.sentence_encoder(
            src_tokens,
        ...
        # T x B x C -> B x T x C
        features = encoder_out.transpose(0, 1)
