...
class Trainer(object):
    ...
    def load_checkpoint(
        ...
        if extra_state is not None:
            ...
            enable_load_from_given_ckpt = getattr(self.args.dataset,'enable_load_from_given_ckpt', False)
            if enable_load_from_given_ckpt:
                if self.args.model.ft_last_f_dir.endswith('.pt'):
                    f_checkpoint_path = self.args.model.ft_last_f_dir
                else: 
                    f_checkpoint_path = os.path.join(self.args.model.ft_last_f_dir, "checkpoint_best.pt") 
                ...
                logger.info(">>> will load all model params.")
            ...
            elif '_2stage' in self.args.model.xlmr_task:    
                f_checkpoint_path = os.path.join(self.args.model.ft_last_f_dir, "checkpoint_best.pt") 
                f_state = torch.load(f_checkpoint_path, map_location=lambda s, l: default_restore_location(s, 'cpu'))
                model_f_dict = self.get_model().state_dict()
                base_dict = {k:v for k,v in f_state['model'].items() if (k in model_f_dict) and ('xlmr' not in k) and ('embed_tokens' not in k)}
                model_f_dict.update(base_dict)
                self.get_model().load_state_dict(model_f_dict) 
                logger.info(">>> successful load 2stage ckpt and start to train.")
            else:
    ...
    def _fast_stat_sync_sum(
        ...
        *logging_outputs* cannot contain any nested dicts/lists.
        ...
        if len(logging_outputs) > 0:
            ...
            for k in log_keys:
                if not ignore:
                    v = sum(log[k] for log in logging_outputs if k in log and log[k] is not None)
                else:
