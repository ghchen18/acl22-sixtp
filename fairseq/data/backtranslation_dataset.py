...
def backtranslate_samples(samples, collate_fn, generate_fn, cuda=True, tgt_lang_id = None, bos = 0, cuda_id = None):
    ...
    bsz, src_len = collated_samples['net_input']['src_tokens'].size()
    collated_samples['net_input']['src_tokens'] = torch.cat((torch.tensor([bos]).repeat(bsz,1), collated_samples['net_input']['src_tokens']),-1)
    batch_sample = utils.move_to_cuda(collated_samples, device=f'cuda:{cuda_id}') if cuda else collated_samples
    batch_sample['net_input']['tgt_lang_id'] = torch.tensor([tgt_lang_id]).type_as(batch_sample['net_input']['src_tokens'])

    # print(f">>> BTdataset.py Now start to translate BT to {tgt_lang_id} ")
    generated_sources = generate_fn(batch_sample)
    ...
    return [
        {
            ...
            "source": torch.cat([torch.LongTensor([bos]), hypos[0]["tokens"].cpu()]),
        }
...
class BacktranslationDataset(FairseqDataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.cfg = cfg
        self.cuda_id = cfg.distributed_training.distributed_rank if hasattr(cfg, 'distributed_training') else 0
    ...
    def __getitem__(self, index):
        ...
        """
        try:
            sample = self.tgt_dataset[index]
        except:
            self.tgt_dataset.prefetch([index])
            sample = self.tgt_dataset[index]
        return sample
    ...
    def collater(self, samples):
        ...
        out_samples = self.output_collater(samples)
        return out_samples
