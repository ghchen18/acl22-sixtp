...
class MultilingualDatasetManager(object):
    def __init__(self, args, lang_pairs, langs, dicts, sampling_method, truncate_source=False, truncate_target=False, cfg=None):
        ...
        self.xlmr_task = getattr(self.args, 'xlmr_task', None) if self.args is not None else None 
    ...
    def load_lang_dataset(
        ...
        enable_xlmr_bos = ('mbart' not in self.args.xlmr_task) and ('vanilla' not in self.args.xlmr_task) and (('encfix_embfix_decrand' in self.args.xlmr_task) or ('xlmr_2stage' in self.args.xlmr_task)) 

        if enable_xlmr_bos:
            assert hasattr(src_dict, "bos_index") 
            src_dataset = PrependTokenDataset(src_dataset, src_dict.bos())
            logger.info(">>> Now Add BOS to src tokens ...")
        elif prepend_bos:
    ...
    def load_a_dataset(
        ...
        **extra_kwargs,
        ...
        if hasattr(self.args, 'real_tgt_lang') and self.args.real_tgt_lang is not None:
            src_lang = src.replace('src', self.args.real_src_lang)
            tgt_lang = tgt.replace('tgt', self.args.real_tgt_lang)
        else:
            src_lang = src 
            tgt_lang = tgt
